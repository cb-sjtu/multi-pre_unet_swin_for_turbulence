#!/bin/bash
# Training script for flow_lstm_3plane model (12-channel version)
# Train + evaluate, and make sure evaluation logs go into the SAME run directory.

echo "Starting training for flow_lstm_3plane model..."

# 1) 训练
echo "Starting training..."
python src/train.py --config-name=train_flow_lstm_3plane trainer.max_steps=20000 trainer.val_check_interval=300 trainer.limit_val_batches=5
...
LATEST_RUN=$(ls -t logs/flow_lstm_3plane/runs/ | head -1)
if [ -z "$LATEST_RUN" ]; then
    echo "No run directory found under logs/flow_lstm_3plane/runs/"
    exit 1
fi

RUN_DIR="logs/flow_lstm_3plane/runs/${LATEST_RUN}"
