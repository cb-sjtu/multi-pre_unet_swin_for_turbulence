#!/bin/bash
# Training script for flow_fno_1plane model (3-channel version using FNO)
# Single y-plane (yslice54) with uvw channels at 256×256 resolution
# Train + evaluate, and make sure evaluation logs go into the SAME run directory.

echo "Starting training for flow_fno_1plane model..."

# 1) 训练
echo "Starting training..."
python src/train.py --config-name=train_flow_fno_1plane trainer.max_steps=50000 trainer.val_check_interval=300 trainer.limit_val_batches=5
...
LATEST_RUN=$(ls -t logs/flow_fno_1plane/runs/ | head -1)
if [ -z "$LATEST_RUN" ]; then
    echo "No run directory found under logs/flow_fno_1plane/runs/"
    exit 1
fi

RUN_DIR="logs/flow_fno_1plane/runs/${LATEST_RUN}"
