...
class OnePlaneModelEvaluator:
    ...
    def _setup_datasets(self):
        ...
        y_slice = 54  # Single y-plane for 1-plane model
    ...
    def visualize_1plane_prediction(self, sample_idx: int = 0, num_future: int = 20):
        ...
        y_slice = channel_info.get("y_slice", 54)  # Single y-slice for 1-plane
        y_slices = channel_info.get("y_slices", [y_slice])  # Compatibility wrapper
        num_planes = channel_info.get("num_planes", 1)  # 1 for 1-plane
    ...
    def _create_comprehensive_comparison(
        ...
        plt.suptitle(f"1-Plane 4-Channel Prediction Overview - Sample {sample_idx}", fontsize=14)
        plt.tight_layout()
    ...
    def create_1plane_animation(self, sample_idx: int = 0, num_future: int = 20):
        ...
        y_slice = channel_info.get("y_slice", 54)  # Single y-slice for 1-plane
        y_slices = channel_info.get("y_slices", [y_slice])  # Compatibility wrapper
        num_planes = channel_info.get("num_planes", 1)
    ...
    def run_comprehensive_evaluation(self, num_samples: int = 1, num_future: int = 20):
        ...
        print("- Individual channel images (12 files per sample: 1 plane × 4 channels)")
    ...
    def visualize_autoregressive(self, split: str = "train", sample_idx: int = 0, num_future: int = 40):
        ...
        channel_info = dataset.get_channel_info()
        field_names = channel_info["field_names"]
        y_slice = channel_info.get("y_slice", 54)
        y_slices = channel_info.get("y_slices", [y_slice])
        num_planes = channel_info.get("num_planes", 1)
    ...
    def _save_teacher_forcing_errors(self, ground_truth_frames, predictions, split, sample_idx):
        ...
        y_slice = channel_info.get("y_slice", 54)
        y_slices = channel_info.get("y_slices", [y_slice])
        num_planes = len(y_slices) if isinstance(y_slices, list) else 1
    ...
    def _create_1plane_comparison_plot(self, ground_truth_frames, predictions, mode, sample_idx):
        ...
        # Create 3x4 subplot (1 plane x 4 fields)
        fig, axes = plt.subplots(3, 4, figsize=(20, 15))
        fig.suptitle(f"1-Plane Comparison: {mode} (Sample {sample_idx})", fontsize=16)
    ...
    def _compute_energy_spectra(self, frames, field_names, y_slices, dx=1.0, dz=1.0):
        ...
        Args:
            frames: Tensor of shape (T, C, H, W) where C=12 (1 plane × 4 fields)
...
def main():
    ...
    if args.checkpoint_path:
        ...
        checkpoint_path = (
            ...
            "runs/2025-11-05_20-32-55-526491/checkpoints/step_6300.ckpt"
        )
